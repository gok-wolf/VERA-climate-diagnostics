...
# Sitta krueperi single-species tutorial pipeline (19- and 36-predictor dual run)
# -----------------------------------------------------------------------------
# Self-contained script (no external config file).
...
# session info, output inventory -- for a single focal taxon under both the
# 19-predictor (Bio1-Bio19) and the 36-predictor (Bio + Envirems) profiles.
#
...
# 19-profile results : C:/VERA/Results/19/Skr_current
# 36-profile results : C:/VERA/Results/36/Skr_current
#
...
cfg <- list(
  ...
  # Dual-profile run: 19 predictors first, then 36 predictors.
  predictor_profiles                = c(19L, 36L),

  # Initial profile used during first configuration validation. The loop at
  # the bottom of the script subsequently executes both 19L and 36L in order.
  predictor_profile                 = 19L,

  # Canonical predictor sets identical to the multi-species pipeline.
  predictor_sets       = list(
    `19` = paste0("Bio", 1:19),
  ...
  # Result depot roots (per predictor profile).
  output_roots         = c(
  ...
  # Populated at run time by the dual-profile loop.
  ...
  # Populated at run time from predictor_sets[[profile_key]].
  focal_vars           = NULL,
...
# Fix the first working profile so the initial validate_config() call has a
# well-formed cfg. The dual-profile loop below rewrites these fields per pass.
profile_key <- as.character(cfg$predictor_profile)
if (!profile_key %in% names(cfg$predictor_sets)) {
  stop("predictor_profile must be either 19L or 36L.")
}
cfg$focal_vars <- cfg$predictor_sets[[profile_key]]
cfg$output_dir <- file.path(unname(cfg$output_roots[[profile_key]]),
                            cfg$output_suffix)
...
validate_config <- function(cfg) {
  ...
  profile_key <- as.character(cfg$predictor_profile)
  if (!profile_key %in% c("19", "36")) {
    stop("predictor_profile must be either 19L or 36L.")
  }
  if (length(cfg$focal_vars) != as.integer(profile_key)) {
    stop("focal_vars does not match predictor_profile.")
  }
...
prepare_stack <- function(cfg) {
  ...
  cat(sprintf("\n>>> Building raster stack for the %s-predictor profile...\n",
              as.character(cfg$predictor_profile)))
...
invisible(list(
  ...
  predictor_profile = cfg$predictor_profile,
...
# =============================================================================
# SECTION 13 -- SITTA KRUEPERI DUAL-PROFILE EXECUTION LOOP
# =============================================================================
# Runs the same focal taxon under both the 19- and 36-predictor profiles.
# Each pass rebuilds the raster stack for its own profile and writes outputs
# to the corresponding results depot (C:/VERA/Results/19 or /36).
# =============================================================================

profiles_to_run <- unique(as.integer(cfg$predictor_profiles))
if (!length(profiles_to_run) || any(!profiles_to_run %in% c(19L, 36L))) {
  stop("predictor_profiles must contain 19L, 36L, or both.")
}
...
single_species_results <- vector("list", length(profiles_to_run))

for (idx in seq_along(profiles_to_run)) {
  profile_value <- profiles_to_run[idx]
  profile_key <- as.character(profile_value)

  cfg$predictor_profile <- profile_value
  cfg$focal_vars        <- cfg$predictor_sets[[profile_key]]
  cfg$output_dir        <- file.path(unname(cfg$output_roots[[profile_key]]),
                                     cfg$output_suffix)
  ...
  output_root <- unname(cfg$output_roots[[profile_key]])
  if (!dir.exists(output_root)) {
    dir.create(output_root, recursive = TRUE, showWarnings = FALSE)
  }

  cat("\n========================================================================\n")
  cat(sprintf("VERA TUTORIAL RUN  [%d / %d]\n", idx, length(profiles_to_run)))
  cat(sprintf("Focal taxon        : %s (%s)\n", cfg$species_label, cfg$species_code))
  cat(sprintf("Predictor profile  : %s variables\n", profile_key))
  ...
  single_species_results[[idx]] <- tryCatch(
    run_vera_current(cfg, prepared_stack),
    error = function(e) {
      message(sprintf("\n!!! VERA failed for %s (profile %s): %s",
                      cfg$species_code, profile_key, conditionMessage(e)))
      list(
        species_code = cfg$species_code,
        species_label = cfg$species_label,
        output_dir = cfg$output_dir,
        predictor_profile = profile_value,
...
# -----------------------------------------------------------------------------
# Combined summary of both profiles for the focal taxon.
# -----------------------------------------------------------------------------
batch_summary <- dplyr::bind_rows(lapply(single_species_results, function(result) {
  tibble::tibble(
    species_code = result$species_code,
    species_label = result$species_label,
    predictor_profile = result$predictor_profile,
...
# One combined summary is written next to each profile's output root so both
# depots (C:/VERA/Results/19 and /36) keep a self-contained record.
for (profile_value in profiles_to_run) {
  profile_key <- as.character(profile_value)
  output_root <- unname(cfg$output_roots[[profile_key]])
  summary_file <- file.path(
    output_root,
    paste0(cfg$species_code, "_dual_profile_run_summary.csv")
  )
  write.csv(batch_summary, summary_file, row.names = FALSE)
  cat(sprintf("\nDual-profile summary written to: %s\n", summary_file))
}

cat("\n>>> SITTA KRUEPERI DUAL-PROFILE SUMMARY\n")
print(batch_summary)

invisible(batch_summary)